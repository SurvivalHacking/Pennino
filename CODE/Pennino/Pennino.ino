...
#define SERVOFAKTORLEFT 1140  // Servo factor for the left motor to convert angle to PWM
#define SERVOFAKTORRIGHT 830  // Servo factor for the right motor to convert angle to PWM
#define SERVOLEFTNULL 1950    // Neutral position for the left servo motor (in microseconds)
#define SERVORIGHTNULL 815    // Neutral position for the right servo motor (in microseconds)
...
#define L1 55  // Length of the first segment of the L arm (in mm)
#define L2 82  // Length of the second segment of the L arm (in mm)
#define L3 13  // Length of the third segment of the R arm (in mm)
#define L4 70  // Length of the fourth segment of the R arm (in mm)

// Arm offsets for the starting positions of the servos
#define O1X 35  // X-coordinate of the L servo center (in mm)
#define O1Y -55 // Y-coordinate of the L servo center (in mm)
#define O2X 83  // X-coordinate of the R servo center(in mm)
...
volatile double lastX = 75;   // Last X position of the arm
volatile double lastY = 47.5; // Last Y position of the arm
...
void setup() 
{
  ...
  // Initialize pin for clearing display (if used)
  pinMode(CLEARPIN, OUTPUT);  
  CleanScreen();  // Clear the display at startup
  number(1, 1, 222, 1);  // Display home screen (or default starting display)
...
{ 
  ...
  if (last_min != timeinfo.tm_min) {
    lift(0);                    // Upper the pen to stop drawing
    ...
    number(1, 1, 111, 1);  // Display a colon separator
    number(-7, Ypos, i, factorH);  // Display the first digit of the hour
    number(9, Ypos, (h - i * 10), factorH);  // Display the second digit of the hour
    number(22, Ypos, 11, factorH);  // Display the colon separator

    // Display minute digits
    i = 0;
    while ((i + 1) * 10 <= m) i++;  // Determine the first digit of the minute
    number(32, Ypos, i, factorM);  // Display the first digit of the minute
    number(50, Ypos + 5, (m - i * 10), factorM);  // Display the second digit of the minute

    // Reset display to home screen after a brief delay
    number(1, 1, 222, 1);  // Return display to the default state
    ...
    delay(10000);  // If no update is needed, wait for 10 seconds
  }
#endif
}
...
void calibrateServos() {
  if (FirstTime==0) {  // Moves servos to predefined positions for 10 seconds
    ...
    drawTo(-3, 29.2);  // Move arm to a predefined coordinate
    delay(10000);  
    FirstTime=1;
  }
  ...
  drawTo(74.1, 28);  // Move arm to another coordinate
  delay(500);        // Short delay between movements
}
...
void set_XY(double Tx, double Ty) {
  ...
  Hx = Tx + L3 * cos((a1 - a2 + 0.621) + M_PI);  // Calculate the X position for right servo movement
  Hy = Ty + L3 * sin((a1 - a2 + 0.621) + M_PI);  // Calculate the Y position for right servo movement
...
void number(float bx, float by, int num, float scale) {
    ...
    case 0:
      // Draw from (bx + 12*scale, by + 6*scale) position to the next point.
      drawTo(bx + 12 * scale, by + 6 * scale);
      lift(0);  // Lower the pen to continue drawing
      // Draw the curve (arc) from (bx + 7*scale, by + 10*scale) with certain curvature and scale
    ...
    case 1:
      // Draw from (bx + 3*scale, by + 15*scale) to (bx + 10*scale, by + 20*scale) and then down
    ...
    case 2:
      // Draw starting from (bx + 2*scale, by + 12*scale)
      drawTo(bx + 2 * scale, by + 12 * scale);
      lift(0);
      // Draw an upward arc (bogenUZS) from (bx + 8*scale, by + 14*scale)
      bogenUZS(bx + 8 * scale, by + 14 * scale, 6 * scale, 3, -0.8, 1);
      // Complete the number '2' by drawing horizontal lines
    ...
    case 3:
      // Start drawing from (bx + 2*scale, by + 17*scale)
      drawTo(bx + 2 * scale, by + 17 * scale);
      lift(0);
      // Draw two arcs to form the number '3'
    ...
    case 4:
      // Start from the bottom-left of the number '4'
      drawTo(bx + 10 * scale, by + 0 * scale);
      lift(0);
      // Draw vertical and diagonal lines forming the number '4'
      drawTo(bx + 10 * scale, by + 20 * scale);
    ...
    case 5:
      // Start drawing from (bx + 2*scale, by + 5*scale)
      drawTo(bx + 2 * scale, by + 5 * scale);
      lift(0);
      // Draw an arc to form part of the number '5'
      bogenGZS(bx + 5 * scale, by + 6 * scale, 6 * scale, -2.5, 2, 1);
      // Complete the number '5' by drawing lines
    ...
    case 6:
      // Start from (bx + 2*scale, by + 10*scale)
      drawTo(bx + 2 * scale, by + 10 * scale);
      lift(0);
      // Draw an arc to form the upper part of the number '6'
      bogenUZS(bx + 7 * scale, by + 6 * scale, 6 * scale, 2, -4.4, 1);
      // Complete the number '6' by drawing the rest of the lines
    ...
    case 7:
      // Start from the top-left of the number '7'
      drawTo(bx + 2 * scale, by + 20 * scale);
      lift(0);
      // Draw the top line and then the diagonal line forming the '7'
      drawTo(bx + 12 * scale, by + 20 * scale);
    ...
    case 8:
      // Start from (bx + 5*scale, by + 10*scale)
      drawTo(bx + 5 * scale, by + 10 * scale);
      lift(0);
      // Draw two arcs forming the top and bottom of the number '8'
    ...
    case 9:
      // Start from (bx + 9*scale, by + 11*scale)
      drawTo(bx + 9 * scale, by + 11 * scale);
      lift(0);
      // Draw an arc to form the upper part of the number '9'
      bogenUZS(bx + 7 * scale, by + 15 * scale, 5 * scale, 4, -0.5, 1);
      // Complete the number '9' by drawing the bottom line
    ...
    case 222:
      lift(1);  // Lift the pen to stop drawing
      drawTo(-3.0, 20.0);  // Move the pen to a pre-defined home position (coordinates are (-3.0, 20.0))
      break;

    // Case for drawing a dot (for '11')
    case 11:
      // Start drawing a dot at (bx + 5*scale, by + 15*scale)
      drawTo(bx + 5 * scale, by + 15 * scale);
      lift(0);  // Lift the pen for smooth transition into arc
      // Draw a small arc to represent the dot
      bogenGZS(bx + 5 * scale, by + 15 * scale, 0.1 * scale, 1, -1, 1);
      delay(10);  // Add a small delay for smoother drawing
      lift(1);  // Lift the pen after finishing the dot
      // Move to the bottom of the dot and draw another arc for the lower part
      drawTo(bx + 5 * scale, by + 5 * scale);
      lift(0);
      bogenGZS(bx + 5 * scale, by + 5 * scale, 0.1 * scale, 1, -1, 1);
      delay(10);  // Delay again to ensure smooth drawing
      lift(1);  // Lift the pen again to complete the operation
      break;
  }
}
